...
export default {
    // 登录提交
    cloginIn(data){
        let url = GLOBAL.URL+"/main/clogin3/login.json";
    ...
    // 登录短信验证码
    cloginIdentCode(data){
        let url = GLOBAL.URL+"/main/clogin3/getSliderForLogin.json";
        let params = data.data;
        if(data.channel=="hxdapp"){
            return hxdRESTPluginPost(url,params);
        }
    ...
    // 登录滑块验证码
    cloginImgCode(data){
        let url = GLOBAL.URL+"/main/clogin3/sendLoginMessage.json";
        let params = data.data;
        if(data.channel=="hxdapp"){
            return hxdRESTPluginPost(url,params);
        }
