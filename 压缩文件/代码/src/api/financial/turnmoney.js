...
export default {
 // 转入首页接口信息
 turnHome(data){
  let  url = HTTP_URL+'api2/wealth/fundRollInAndRollOut/getInCardList.json';
  let params = data;
...
// 确定转入接口
subEnter(data){
    let  url = HTTP_URL+'api2/wealth/fundRollInAndRollOut/fundRollIn.json'
    let params = data;
    if(params.channel=='hxdapp'){
        return hxdRESTPluginPost(url,params);
      }
  ...
  // ----------------------------------
   // 转出首页接口信息
 outHome(data){
  let  url = HTTP_URL+'api2/wealth/fundRollInAndRollOut/getOutCardList.json';
  let params = data;
  if(params.channel=='hxdapp'){
    return hxdRESTPluginPost(url,params);
  }
...
// 确定转出接口
outEnter(data){
  let  url = HTTP_URL+'api2/wealth/fundRollInAndRollOut/fundRollOut.json'
  let params = data;
  if(params.channel=='hxdapp'){
      return hxdRESTPluginPost(url,params);
    }
