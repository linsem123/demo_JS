...
export default {
  name: "BdebitSelect",
  props: ['nocard','hascard',"choseShow",'bankName','bankNum','debitList'],
  data(){
    return{
      debitShow:false,
  ...
  methods: {
    ...
    changeChosed(item,index){     // 选择借记卡
      ...
      //   if(index==i){
          ...
          this.$emit("getdebitcard",{data:item,index:index}); // 返回被选中的借记卡信息
      //   }
      // });
    ...
    Remove(index,item){
      ...
      this.removeCard = {"cardIndex":index,"cardNum":item};
    ...
    btnConfirm(){
      this.deleteShow = false;
      this.$emit("btnConfirm",this.removeCard);
...
<style scoped>
/* 添加借记卡 */
.lend-content{padding:0.31rem 5.3% 0.17rem;box-sizing: border-box;}
...
.lend-content .cell-middle{float:right;}
.lend-content .cell-middle input{border:none;;font-size: 0.15rem;text-align: left;height: 0.24rem;line-height: 0.24rem;width:1.8rem;}
...
.lend-content .cell-right img{width:0.09rem;}
.lend-content .lend-cell.te2{margin:0;}
/* 借记卡列表 */
.debitcard-select {background: #fff;width: 100%;height: 2.66rem;z-index: 600;position: fixed;bottom: 0;color: #333;}
...
/* 借记卡删除 */
.deb-remind{position: absolute;top:50%;width:2.71rem;left: 50%;height: 1.07rem;transform:translate(-50%,-100%);background: #FFFFFF;border-radius: 0.1rem;
z-index:999;}
...
#bankName,#bankNum,#addAmount{text-align: right;margin-right:0.1rem;}
#bankNum{margin-right:0.2rem;}
</style>
