import entry from "@/api/entry.js";
...
const actions = {
  ...
  getphonecode({dispatch,commit,state},payload){ 
    ...
    entry.sendSmsCode(baseData).then((res)=>{
      ...
      if(res.success==true){
        if(res.data.needSlider==false){                   // false=>隐藏滑块
          ...
          entry.getSlider(baseData).then((res)=>{
            if(res.success==true){
...
const mutations={
  BASSINID(state,data){             // 证件号码赋值
    state.entryIn.entryData.IdNum = data.IdNum;
  },
  BASSINPHONE(state,data){          // 手机号赋值
    state.entryIn.entryData.PhoneNum = data.PhoneNum;
  },
  BSHOWINDENTMSG(state,data){       // 显示证件号码错误提示
    state.entryIn.entryData.remindmsg.indentMsg = data.msg;
  },
  BSHOWPHONEMSG(state,data){        // 显示手机号错误提示
    state.entryIn.entryData.remindmsg.phoneMsg = data.msg;
  },
  BSHOWCODEMSG(state,data){         // 显示验证码错误提示
    state.entryIn.entryData.remindmsg.codeMsg = data.msg;
  },
  BSHOWLOGINALERT(state,data){      // 登录页面信息弹框
    state.entryIn.alert.showAlert = data.show;
    state.entryIn.alert.msg = data.msg;
    state.entryIn.alert.err = data.err;
  },
  BIMAGECODEMOVE(state,value){      // 滑块滑动距离赋值 默认是0
    state.entryIn.identify.imageCode.sendlen = value;
  }, 
  BSHOWIMAGECODE(state, value) {    // 判断是否出现滑块验证码 默认false
    state.entryIn.identify.imageCode.show = value;
  },
  BCHANGESTATUS(state, value) {     // 更改按钮状态。防止多次点击
    state.entryIn.identify.textMsg.status = value;
  },
  BCHANGETEXT(state, data) {        // 更换按钮文字
    state.entryIn.identify.textMsg.msg = data;
  },
  SETINTERVAL(state, value) {       // 设置倒计时时间
    state.entryIn.identify.textMsg.t = value;
  },
  BASSIGNIMAGE(state,res){          // 给滑块验证码图片赋值
    state.entryIn.identify.imageCode.bgImg = res.bigimgStr;
    state.entryIn.identify.imageCode.slideImg = res.tarimgStr;
  },
};
