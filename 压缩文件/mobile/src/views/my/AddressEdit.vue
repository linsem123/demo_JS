<template>
  <div>
    ...
    >
      <div slot="left"></div>
      <div slot="main">地址编辑</div>
      <div slot="right"></div>
    </top-head>
    <!-- 头部 -->


    <div class="mall-edit-address">
...
import { Toast, Indicator, Switch, Picker, MessageBox } from "mint-ui";
...
export default {
  ...
  methods: {
    init: function() {
      ...
      // utils.getCurrentPosition()//获取当前ip的经纬度
      // .then(function(res){
      //   if(res){
      //     _this.position = _.assign(_this.position,res);
      //   }
      // })
      //
      this.getMyAddressDetail(function() {
    ...
    getMyAddressDetail: function(cb) {
      var _this = this;
      utils.API.address_info({
        data: {
          token: utils.storageGet("mall_token"),
          address_id: _this.$route.params.id
        }
      })
        .then(function(ret) {
          _this.address_detail = ret.model.data;
          _this.defalut_address_detail = _.assign({}, ret.model.data);

          var address_head_info =
            _this.address_detail.country_name +
            _this.address_detail.province_name +
            _this.address_detail.city_name +
            _this.address_detail.district_name +
            _this.address_detail.street_name;
          var detail = "";
          if (
            new RegExp(address_head_info, "g").test(_this.address_detail.address)
          ) {
            detail = _this.address_detail.address.split(address_head_info)[1];
          } else {
            detail = _this.address_detail.address;
          }

          _this.info_data = _.assign(_this.info_data, {
            consignee: _this.address_detail.consignee, //收件人
            mobile: _this.address_detail.mobile, //手机
            address_head_info: address_head_info, //选择的省市、区县
            address_detail: detail || "" //详细地址
          });

          _this.set_default = _this.address_detail.default_address==1?true:false;
          cb && cb();
    ...
    validate: function(key) {
      return (this.validation[key] = new RegExp(
        constants.FORM_REG[key],
        "g"
      ).test(_.trim(this.info_data[key])));
    },
    ...
    confirmAddressPicker: function() {
      ...
      this.info_data.address_head_info =
    ...
    onValuesChange(picker, values) {
      ...
      _this.district = _.assign({}, _this.district, {
        id: _.result(
          _.find(_this.district_arr, function(o) {
            return o.name == values[2];
          }),
          "id"
        ),
        name: values[2]
      ...
      MessageBox({
        ...
        if (action === "confirm") {
          utils.API.address_delete({
            data: {
              token: utils.storageGet("mall_token"),
              address_id: _this.$route.params.address_id
            }
          })
            .then(function(ret) {
              _this.getMyAddress(function() {
                _this.goBack();
              });
            })
            .catch(function(ret) {
    ...
    save: function() {
      ...
      utils.API.address_update({
        ...
        .then(function(ret) {
          ...
          // _this.$store.commit('setShopCategory',ret.model.data);
          // console.log("ret:",ret)
          // _this.$router.push({path: _this.referrer });
          Toast("地址编辑成功！");
          _this.$router.goBack();
        })
        .catch(function(ret) {
