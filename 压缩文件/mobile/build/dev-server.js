...
const {hcp_config, PFC_CODE, ctx_config} = ctxEnvConfig[ctxEnv];


fs.writeFile(path.resolve(__dirname, "../src/config/appconfig.js"), 
`
"use strict";
module.exports = ${JSON.stringify(merge(appConfig, ctx_config,{PFC_CODE:PFC_CODE,AUTO_UPDATE:false}, {debug:true, env:ctxEnv}))};
`, function(err) {
  if (err) {
    console.error(err);
  } else {
